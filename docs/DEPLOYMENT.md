...
> 💡 **Lesson learned:** **The first GitHub Advanced Security scan is a baseline, not an incident.** Dependabot High on a FilePress or Wrangler site lockfile is usually `sharp` (libheif) or Ajv’s `fast-uri`. Override the named package in that lockfile. Leave cookie 1.x alone; only lift `cookie@<0.7.0`. Do not open a GHSA for a lockfile pin.
