...
- [ ] **Dependabot High on lockfiles:** Pin the **named** transitive with `pnpm.overrides` (or npm `overrides`) in the package that owns that lockfile. Do **not** major-upgrade Vite 4 / SvelteKit 1 (or another EOL framework line) just to pick up a nested advisory. Recurring pins as of 2026-09: `sharp` `0.35.4` (libheif), `fast-uri` `3.1.7` (Ajv), `cookie@<0.7.0` → `0.7.2`, `js-yaml` `3.15.2`, `brace-expansion` `5.0.9` (or `1.1.18` / `2.1.4` / `3.0.6` on older majors), `launch-editor` `2.14.1`, `qs` `6.16.0`, `hono` `4.13.7`. Cookie 1.x already sits past the 0.6 advisory; leave it. Vitest 3.x has **no** patch for the `@vitest/mocker` path-traversal advisory: bump the **devDependency** to `4.1.11` (not a product-framework jump). Vite 4 / Kit 1 / Svelte 3 / devalue 4 / undici 5 have **no patch on that major**: dismiss as “no patch on this major; local/dev-only” after the pin pass.
...
> 💡 **Lesson learned:** **Dependabot High on a site or tool lockfile is usually a nested pin, not a product rewrite.** FilePress sites pull `sharp`; Ajv pulls `fast-uri`; SvelteKit still ships `cookie@0.6` next to `cookie@1`. Add `pnpm.overrides` in the package that owns the flagged lockfile, run `pnpm install --lockfile-only`, and leave the app version and the site framework where they are. A fleet scan of lockfiles (skip `node_modules`) finds the same four packages on almost every marketing site. A first-scan dump on an old SvelteKit 1 / Vite 4 client looks huge and “might be old”: most of those Highs have no 4.x patch. Pin what still has a same-major fix (`launch-editor`, `qs`, `hono`) and dismiss the rest until a planned Kit 2 rewrite.
>
> 📝 **Example:** `"pnpm": { "overrides": { "sharp": "0.35.4", "fast-uri": "3.1.7", "cookie@<0.7.0": "0.7.2", "brace-expansion": "5.0.9" } }`
