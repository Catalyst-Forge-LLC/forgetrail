...
- **Major-upgrading a framework to clear a nested Dependabot High.** Pin the named package with `pnpm.overrides` in the flagged lockfile (`sharp` `0.35.4`, `fast-uri` `3.1.7`, `cookie@<0.7.0` → `0.7.2`, `brace-expansion` `5.0.9`). Leave Vite 4 / SvelteKit 1 where they are unless you meant to take that upgrade.
