...
- **Major-upgrading a framework to clear a nested Dependabot High.** Pin the named package with `pnpm.overrides` in the flagged lockfile (`sharp` `0.35.4`, `fast-uri` `3.1.7`, `cookie@<0.7.0` → `0.7.2`, `brace-expansion` `5.0.9`, `launch-editor` `2.14.1`). Leave Vite 4 / SvelteKit 1 where they are unless you meant to take that upgrade. Dismiss leftover Vite 4 / Kit 1 / Svelte 3 / devalue 4 / undici 5 alerts (no patch on that major).
